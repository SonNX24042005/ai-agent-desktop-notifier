...
Write-Host "Building the Rust runtime..."
Push-Location $ProjectRoot
try {
    cargo build --release -p anoti-app
    if ($LASTEXITCODE -ne 0) { throw "Cargo build failed." }
    & "$ProjectRoot\target\release\anoti.exe" install
    if ($LASTEXITCODE -ne 0) { throw "Rust installer failed." }
} finally {
    Pop-Location
...
Write-Host "The Rust runtime and managed hooks are installed."
